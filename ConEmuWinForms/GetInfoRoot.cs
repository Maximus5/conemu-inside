...
namespace ConEmu.WinForms
{
	...
	{
		private GetInfoRoot([NotNull] string name, int? pid, bool isRunning, int? exitCode)
		{
			if(name == null)
		...
		public static Task<GetInfoRoot> QueryAsync([NotNull] ConEmuSession session)
		{
			...
			return session.BeginGuiMacro("GetInfo").WithParam("Root").ExecuteAsync().ContinueWith(task =>
			{
				...
				GuiMacroResult result = task.Result;
				...
				int nPidRaw;
				int? nPid = ((isRunning == true) && (int.TryParse(xmlRoot.GetAttribute("PID"), NumberStyles.Integer, NumberFormatInfo.InvariantInfo, out nPidRaw))) ? nPidRaw : default(int?);
